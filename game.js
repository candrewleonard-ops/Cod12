...
function buildMegaGate(){
  ...
  const tag=new T.Mesh(new T.PlaneGeometry(15,3.4), new T.MeshBasicMaterial({map:KIT.label('OPEN ◈','#ffe9a0'),transparent:true})); tag.position.set(0,49,0.4); tag.rotation.y=Math.PI; g.add(tag);
  ...
  const gate=addInteractable({ x:gx, z:gz, radius:15, type:'gate', cost:0, open:false, planks:[], anim:0,
    label:()=> gate.open?null:{key:'F', txt:'Open the Mega-Gate', cost:0},
    run:()=>{ if(gate.open) return false; gate.open=true; g.visible=false; AU.power();
      toast('MEGA-GATE OPEN','the forbidden area is open','#ffc24a'); return true; } });
